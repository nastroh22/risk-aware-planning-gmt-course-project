...
class Visualizer(object):
    def __init__(self, dt=0.5, fullscreen=False, name='unnamed', iters=1000, magnify=1.):
        ...
        self.window = pyglet.window.Window(800, 600, fullscreen=fullscreen, caption=name)
    ...
    def use_world(self, world):
        ...
        self.exits =  [c for c in world.exits]
    ...
    def draw_lane_surface_og(self, lane):
    ...
    def draw_lane_surface(self, lane):
        ...
        W = 10
        # graphics.draw(4, gl.GL_QUAD_STRIP, ('v2f',
        #     np.hstack([0, 0, 3, 0 , 0,.2, 3,.2,  ])
        # ))
        # np.hstack([.2, .6, .2,.8, .7,1, .9, 1 ])
        theta = 44.7   * (np.pi / 180)
        b = [.2, .1]
        a = [b[0], b[1] + lane.w / (np.cos(theta))]
        d, c = [0,0] , [0,0]
        d[0] = b[0] + np.sin(theta)*W
        d[1] = b[1] + np.cos(theta)*W
        c[0] = a[0] + np.sin(theta)*W
        c[1] = a[1] + np.cos(theta)*W

        graphics.draw(4, gl.GL_QUAD_STRIP, ('v2f',
            np.hstack([a[0], a[1], b[0], b[1], c[0], c[1], d[0], d[1]  ])
        ))
        # print(lane.w / (np.cos(theta)))
        # print(a[1] - b[1])

    def draw_exit(self, lane):

        gl.glColor3f(0.4, 0.4, 0.4)
        W = lane.length 
        graphics.draw(4, gl.GL_QUAD_STRIP, ('v2f',
            np.hstack([lane.p-lane.m*2*W-0.5*lane.w*lane.n , lane.p-lane.m*2*W+0.5*lane.w*lane.n,
                       lane.q+lane.m*W-0.5*lane.w*lane.n,  lane.q+lane.m*W+0.5*lane.w*lane.n])
        ))
        ...
        graphics.draw(4, gl.GL_LINES, ('v2f',
            np.hstack([lane.p-lane.m*W-0.5*lane.w*lane.n, lane.p+lane.m*10*W-0.5*lane.w*lane.n,
                       lane.p-lane.m*W+0.5*lane.w*lane.n, lane.p+lane.m*10*W+0.5*lane.w*lane.n]))
        )
    ...
    def on_draw(self):
        ...
        for lane in self.lanes:
            self.draw_lane_surface_og(lane)
        for lane in self.lanes:
    ...
    def run_sim(self, filename=None, pause_every=None):
