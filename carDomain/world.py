...
import carDomain.dumbcar as dumb
import math
...
def playground():
    ...
    world = World()
    clane = lane.StraightLane([0,  1], [0, -1], 0.17, 1000)

    num_lanes = 3
    lane_width = 0.17
    exit_angle = 70
    exit_rad = exit_angle * (np.pi/180)
    ex1_anchor = [0.17 * 1.5 , .9]  # multiply by half num lanes
    ex1_q = ex1_anchor +  np.array([np.cos(exit_rad) , np.sin(exit_rad)])
    ex2_anchor = [0.17 * 1.6 , -.2]
    ex2_q = ex2_anchor +  np.array([np.cos(exit_rad) , np.sin(exit_rad)])

    ex1 = lane.StraightLane(ex1_anchor,  ex1_q  , 0.17, 0.13)
    ex2 = lane.StraightLane(ex2_anchor,  ex2_q  , 0.17, 0.13)
...
def world0():
    dyn = dynamics.CarDynamics(0.1)
    world = World()
    clane = lane.StraightLane([0., -1.], [0., 1.], 0.13, l = 1000)
    world.lanes += [clane, clane.shifted(1), clane.shifted(-1)]
    world.roads += [clane]
    world.fences += [clane.shifted(2), clane.shifted(-2)]
    world.cars.append(car.UserControlledCar(dyn, [-0.13, 0., math.pi/2., 0.3], color='red'))
...
def passing(flag=False):
    dyn = dynamics.CarDynamics(0.1)
    world = World()
    clane = lane.StraightLane([0., -1.], [0., 1.], 0.13, 1000)
    world.lanes += [clane, clane.shifted(1), clane.shifted(-1)]
    world.roads += [clane]
...
def intersection(flag=False):
    dyn = dynamics.CarDynamics(0.1)
    world = World()
    vlane = lane.StraightLane([0., -1.], [0., 1.], 0.13, 1000)
    hlane = lane.StraightLane([-1., 0.], [1., 0.], 0.13, 1000)
    ...
    # world.cars.append(car.NPC())
    world.cars.append(car.SimpleOptimizerCar(dyn, [-0.3, 0., 0., 0.], color='yellow'))
    world.cars[1].human = world.cars[0]
...
def world5():
    ...
    world.fences += [hlane.shifted(-1), hlane.shifted(1)]
    world.cars.append(car.UserControlledCar(dyn, [0., -.3, math.pi/2., 0.0], color='red'))
    world.cars.append(car.SimpleOptimizerCar(dyn, [-0.3, 0., 0., 0.0], color='yellow'))
    world.cars[1].human = world.cars[0]
    world.cars[1].bounds = [(-3., 3.), (-2., 2.)]
    @feature.feature
    def horizontal(t, x, u):
